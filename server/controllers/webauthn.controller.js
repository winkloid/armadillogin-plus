...
/*
 @desc    get registration response from client (credentialID, public key, etc.) and store it to authenticators_webauthn collection
 @route   POST /api/webauthn/completeRegistration
 @access  Public
 */
...
/*
 @desc    check whether there is an registered user with the specified user name and if so, generate authentication options. For this, request all authenticators connected to the user account from database so that credentialID and authenticator transports can be included in those options
 @route   POST /api/webauthn/authenticationOptions
 @access  Public
*/
...
/*
@desc    Verify whether challenge generated by client matches with information of one of the authenticators connected with the specified user account. For that, first check whether the user actually exists, then request database for an authenticator with the id of the user and the id of the authenticator that was used by the client. Afterwards call verifyAuthenticationResponse to verify the correctness of the data sent by the user agent.
@route   POST /api/webauthn/completeAuthentication
@access  Public
*/
...
/*
@desc    generate registration options, e.g. whether user verification shall be used, type of authenticators, etc. for a new authenticator that is added to an account where one or more authenticators are already connected to
@route   POST /api/webauthn/addNewAuthenticatorOptions
@access  Private
*/
...
/*
 @desc    get registration response from client (credentialID, public key, etc.) and store it to authenticators_webauthn collection - this endpoint is only used if users already registered one or more authenticators to their account and want to add another one
 @route   POST /api/webauthn/addNewAuthenticatorCompletion
 @access  Private
 */
...
/*
@desc    delete the authenticator with the specified credentialID from database so that a user cannot use it again to log in. This only works if the user has at least one authenticator after the deletion process - otherwise the deletion procedure gets cancelled and the original state is restored.
@route   POST /api/webauthn/deleteAuthenticator
@access  Private
 */
...
// @desc create an initial user with the specified user name in user collection
...
// @desc get user information by user name from database
...
// @desc fetch authenticator information from authenticators_webauthn collection by user id and authenticator id
...
// @desc update login counter of the authenticator specified by its id
